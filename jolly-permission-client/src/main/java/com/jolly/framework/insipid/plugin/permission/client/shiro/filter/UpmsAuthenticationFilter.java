package com.jolly.framework.insipid.plugin.permission.client.shiro.filter;

import com.alibaba.fastjson.JSONObject;
import com.jolly.insipid.permission.common.util.PropertiesFileUtil;
import com.jolly.insipid.permission.common.util.RedisUtil;
import com.jolly.framework.insipid.plugin.permission.client.shiro.session.UpmsSessionDao;
import com.jolly.framework.insipid.plugin.permission.client.util.RequestParameterUtil;
import com.jolly.upms.common.constant.UpmsConstant;
...
public class UpmsAuthenticationFilter extends AuthenticationFilter {
    ...
    // 局部会话key
    private final static String ZHENG_UPMS_CLIENT_SESSION_ID = "jolly-upms-client-session-id";
    // 单点同一个code所有局部会话key
    private final static String ZHENG_UPMS_CLIENT_SESSION_IDS = "jolly-upms-client-session-ids";
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        // 判断请求类型
        String upmsType = PropertiesFileUtil.getInstance("jolly-upms-client").get("upms.type");
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        StringBuffer sso_server_url = new StringBuffer(PropertiesFileUtil.getInstance("jolly-upms-client").get("sso.server.url"));
        // server需要登录
        String upmsType = PropertiesFileUtil.getInstance("jolly-upms-client").get("upms.type");
        if ("server".equals(upmsType)) {
        ...
        sso_server_url.append("/sso/index").append("?").append("appid").append("=").append(PropertiesFileUtil.getInstance("jolly-upms-client").get("appID"));
        // 回跳地址
